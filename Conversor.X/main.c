...
ISR(INT0_vect){
   ...
   MostrarNumero(CalcularTension(), 4);
   PORTD = (1<<4);
   _delay_ms(ancho);
   PORTD = (0<<4);
}
...
void MostrarNumero(uint16_t numero, uint8_t digitos)
{
    ...
    uint8_t _milesu = (numero/1000)%10;
    uint8_t _milesd = (numero/1000)%10;
    if(digitos == 3){
      char _tx3[3] ={_centenas + '0',_decenas + '0', _unidades + '0'};
      USART_SetArrayData(_tx3, sizeof _tx3);
    }else if(digitos == 4){
        char _tx4[5] ={_centenas + '0',_decenas + '0',',', _unidades + '0', _milesu + '0' };
        USART_SetArrayData(_tx4, sizeof _tx4);
    }else{
        char _tx5[6] ={_centenas + '0',_decenas + '0', _unidades + '0',',', _milesu + '0', _milesd + '0'};
